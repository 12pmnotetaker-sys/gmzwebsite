...
export function mergeImport(current: State, input: z.infer<typeof importSchema>): State {
  ...
  if (s.dataMode === 'sample') {
    const customPropertyIds = new Set(
      s.properties.filter((p) => !sampleProperties.has(p.id)).map((p) => p.id),
    );
    s.projects = s.projects.filter((p) => !['j1', 'j2'].includes(p.id));
    s.visits = s.visits.filter((v) => !['v1', 'v2', 'v3', 'v4'].includes(v.id));
    ...
    const usedClients = new Set(s.properties.map((p) => p.clientId));
    s.clients = s.clients.filter((c) => !sampleClients.has(c.id) || usedClients.has(c.id));
  }
  for (const c of input.clients) if (!s.clients.some((x) => x.id === c.id)) s.clients.push(c);
  for (const p of input.properties)
    if (!s.properties.some((x) => x.id === p.id)) s.properties.push(p);
  s.dataMode = 'actual';
  s.importIds.push(input.batchId);
  s.activity.unshift({
    id: crypto.randomUUID(),
    at: new Date().toISOString(),
    text: `Imported ${input.properties.length} GMZ property records`,
  });
