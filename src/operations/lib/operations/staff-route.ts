...
export function staffRoute(state: State, book: RouteBook | null, date: string, rotation: string) {
  ...
  return {
    needsRotation:
      ...
      !rotation &&
      !state.routes.some((r) => r.date === date),
    stops: rows.map((row) => ({
      id: row.id,
      ...
      adminNotes: state.serviceRequests
        .filter((r) => r.propertyId === row.propertyId && (r.visitId === row.id || !r.visitId))
        .map((r) => ({ reference: r.reference, note: r.note })),
    })),
  };
}
