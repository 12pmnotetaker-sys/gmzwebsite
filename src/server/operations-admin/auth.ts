...
export async function authenticateAdmin(token: string | undefined): Promise<Admin | null> {
  ...
  const { data: member, error: denied } = await db()
    .from('gmz_operations_admins')
    .select('scope')
  ...
  const { data: integration } = await db()
    .from('gmz_operations_integrations')
    .select('scope')
    .eq('scope', member.scope)
    .eq('active', true)
    .maybeSingle();
  return integration
    ? { userId: data.user.id, scope: member.scope, email: data.user.email || '' }
    : null;
}
